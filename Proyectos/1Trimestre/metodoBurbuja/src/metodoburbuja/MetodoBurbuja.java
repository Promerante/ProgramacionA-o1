...
public class MetodoBurbuja {
    public static void main(String[] args) {             
        int nn; /* Declaración de variables */
    ...
    static void ordenaBurbuja(int ArrayN[]) {
        ...
        for (int i = 0; i < ArrayN.length - 1; i++) {/* Bucle anidado desde 0 hasta la longitud del array -1 */           
            for (int j = 0; j < ArrayN.length - 1; j++) { 
                if (ArrayN[j] > ArrayN[j + 1]) {
                    int temp = ArrayN[j + 1];/* Si el número almacenado en la posición j es mayor que el de la posición j+1 (el siguiente del array) 
